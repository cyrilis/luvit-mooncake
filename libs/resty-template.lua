...
function template.compile(view, key, plain)
    ...
    if key == "no-cache" then
        return load_chunk(template.parse(view, plain, key)), false
    end
    key = key or view
    local cache = template.cache
    if cache[key] then return cache[key], true end
    local func = load_chunk(template.parse(view, plain, key))
    if caching then cache[key] = func end
    return func, false
end

function template.parse(view, plain, key)
    assert(view, "view was not provided for template.parse(view, plain).")
    local concat, rpos, find, byte, sub = concat, rpos, find, byte, sub
    if not plain then
        filePath = view
        baseDir = lpath.dirname(filePath)
        view = template.load(view)
        if byte(sub(view, 1, 1)) == 27 then return view end
    end
    local c = {[[
context=(...) or {}
local function include(v, c, key)
    return template.compile(v, key)(c or context)
end
